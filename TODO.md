...
- [ ] **Map redesign** — Mobbin MCP is now installed (`~/.mobbin-mcp/auth.json`). In a fresh Claude Code session, ask: *"Pull 3-5 Mobbin map UIs from Music & Audio + Live Events categories; propose patterns to evolve `map.jsx` while respecting `DESIGN.md`."* Current map.jsx: 9-stage circular venue with crew pins; could improve gesture-based zoom, stage-detail card, search affordance.
