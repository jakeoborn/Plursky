...
//   · dead URL dropped: an oEmbed failure never reaches a page
...
// ── 3. Selection: verified only, caps, edition window ───────────────────────
{
  ...
  const results = {};
  for (const e of curated.f) results[e.url] = { ok: true, publishedAt: '2026-05-01' };
  results[mk('instagram', 9, 'lineup').url] = { ok: false, reason: 'oEmbed returned 400' };
  results[mk('instagram', 8, 'lineup').url] = { ok: true, publishedAt: '2024-02-27' };
